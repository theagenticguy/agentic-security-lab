# Track C: Sandbox Configs

Copy/paste-ready sandbox definitions for a security-research agent, ordered from least to most isolated. Grounded in Docker, mise, devcontainer-spec, Canonical Workshop, and Firecracker docs.

## 1. Dockerfile — agentic-security image
...
Non-root `agent` (UID 10001), tools on `PATH`. ASan is invoked at build time (`clang -fsanitize=address`), not a package.

## 2. mise.toml — pinned toolchain + tasks
...
`mise run scan|fuzz|harness-gen`. Version pins make the environment reproducible across machines.

## 3. devcontainer.json — hardened runArgs
...
`runArgs` are passed verbatim to `docker run`. `--read-only` root + writable `tmpfs` scratch means any agent-written payload is wiped on teardown. Network is off by default (see §7 to add scoped egress).

## 4. workshop.yaml — Canonical Workshop environment
...
Workshop runs each environment in an unprivileged LXD container; the snapd-inspired `interfaces` system grants only the host resources explicitly listed, so omitting a `display` interface denies GUI access entirely.

## 5. Firecracker — no-network microVM with vsock
...
Read-only rootfs, no `network-interfaces` call (so no NIC at all), and a single vsock UDS for control. Firecracker boots in ~125 ms with ~5 MiB VMM overhead.
